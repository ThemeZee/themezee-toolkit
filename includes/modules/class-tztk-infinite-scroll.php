 ...
 *
 * This class is a fork of the Infinite Scroll Module from JetPack.
 *
...
if ( ! class_exists('TZTK_Infinite_Scroll') ) :

/**
 * Class: TZTK_Infinite_Scroll relies on add_theme_support, expects specific
 * styling from each theme; including fixed footer.
 */
class TZTK_Infinite_Scroll {
	/**
	...
	function action_wp_footer_settings() {
		...
		$click_handle_text = __( 'Older posts', 'themezee-toolkit' );
		...
		if ( is_string( $post_type ) && ! empty( $post_type ) ) {
			...
			if ( is_object( $post_type ) && ! is_wp_error( $post_type ) ) {
				...
				if ( isset( $cpt_text ) ) {
					$click_handle_text = sprintf( __( 'Older %s', 'themezee-toolkit' ), $cpt_text );
					unset( $cpt_text );
				}
			}
		}
		...
		$js_settings = array(
			...
			'totop'            => esc_js( __( 'Scroll back to top', 'themezee-toolkit' ) ),
...
function tztk_infinite_scroll_init() {
	...
	new TZTK_Infinite_Scroll;
}
add_action( 'init', 'tztk_infinite_scroll_init', 20 );

/**
 * Early accommodation of the Infinite Scroll AJAX request
 */
if ( TZTK_Infinite_Scroll::got_infinity() ) {
	/**
