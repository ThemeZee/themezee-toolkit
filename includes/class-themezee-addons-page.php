...
class ThemeZee_Addons_Page {
	...
	*/
	static function add_addons_page() {
		...
		add_theme_page(
			esc_html__( 'ThemeZee Add-ons', 'themezee-toolkit' ),
			esc_html__( 'Theme Add-ons', 'themezee-toolkit' ),
			'manage_options',
			'themezee-addons',
	...
	*/
	static function display_overview_page() { 
		...
		$addon_link = '<a target="_blank" href="http://themezee.com/addons/" title="'. esc_html__( 'ThemeZee Add-ons', 'themezee-toolkit' ) . '">'. esc_html__( 'add-ons', 'themezee-toolkit' ) . '</a>';
		?>
		
		<div id="themezee-addons-overview">
		
			<h1 id="themezee-addon-header"><?php esc_html_e( 'ThemeZee Add-ons', 'themezee-toolkit' ); ?></h1>
			<div class="themezee-addons-intro">
				<?php printf( esc_html__( 'You need more features and functionality? Extend your website with our affordable %s.', 'themezee-toolkit' ), $addon_link ); ?>
			</div>
			<hr/>
